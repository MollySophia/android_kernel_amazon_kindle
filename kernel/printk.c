		      ...
		      const char *text, u16 text_len)
{
	...
	size += print_time(timestamp, NULL);
#endif
	...
	msg->text_len = print_time(msg->ts_nsec, log_text(msg));
...
static int __init log_goal_position(char *str) {
	...
	if(log_goal_location){
		printk(KERN_INFO "%s:%d printk log placed at %08lx\n", __FUNCTION__,__LINE__,log_goal_location);
		return 0;
	}

	log_goal_location = memparse(str, &str);

	printk(KERN_INFO "%s:%d Will try to place printk log at %08lx\n", __FUNCTION__,__LINE__,log_goal_location);
...
static ssize_t backup_buffer_read(struct file *file, char __user *buf,size_t count, loff_t *off)
{

	int i;
	ssize_t written=0;
	
	written = simple_read_from_buffer(buf, count, off, backup_buffer, log_buf_len);
...
void setup_printk_proc() {
        ...
        backup_proc_entry = proc_create("printkbuf", S_IRUGO, NULL,
			&printkbuf_fops);
...
#define LOG_GOAL_LOCATION 0x8fff0000
void printk_reserve_buf(){
	int ret = -EBUSY;

	//reserving bootmem can be placed at very early boot time and parameters can't be parsed at this time, use in-code define LOG_GOAL_LOCATION
	if (!log_goal_location){
	...
	printk(KERN_ERR"%s:%d log_goal_location:0x%x log_worst_location:0x%x new_log_buf_len:%d\n",
		   __FUNCTION__,__LINE__, log_goal_location, log_worst_location,
		   new_log_buf_len);
	
	while (ret < 0 && log_goal_location >= log_worst_location) {
		ret = -EBUSY;

		if(memblock_is_region_memory(log_goal_location, new_log_buf_len) &&
		   !memblock_is_region_reserved(log_goal_location, new_log_buf_len))
		   ret = memblock_reserve(log_goal_location, new_log_buf_len);
...
EXPORT_SYMBOL(printk_reserve_buf);
...
#ifdef CONFIG_LAB126_PRINTK_BUFFER
		printk("Prink buffer will be relocated to physical address: %08lx va: %08lx new_log_buf_len:%d\n", log_goal_location, new_log_buf, new_log_buf_len);
