...
#ifdef CONFIG_LAB126	
static int g_BTstatus =0;
static unsigned int g_BTirq = 0;
#endif
...
#ifdef CONFIG_LAB126
	//when BT is turned off, we switch uart clk source back to low speed clock to save power. The serial_imx_resume resume both console uart and BT uart. Therefore if there are linger data activities on the BT uart bus, it would trigger interrupt when BT is actually off. This would cause watchdog. Therefore it is necessary to ignore the interrupt from BT when BT is off
	if(g_BTstatus == 0 && sport->port.irq == g_BTirq)
		return IRQ_HANDLED;
#endif
...
#ifdef CONFIG_LAB126	
		if(strcmp("2034000.serial", dev_name(port->dev)) == 0) {
			g_BTirq = sport->port.irq;
			dev_dbg(sport->port.dev, "g_BTirq:%d\n", g_BTirq);
		}
#endif

		dev_dbg(sport->port.dev, "%s: serial imx %s request irq:%d\n",__FUNCTION__, dev_name(port->dev), sport->port.irq);
	}
...
static void imx_shutdown(struct uart_port *port)
{
	...
	{
		free_irq(sport->port.irq, sport);
		dev_dbg(sport->port.dev, "%s: serial imx release irq:%d\n",__FUNCTION__, sport->port.irq);
	}
...
extern void bt_uart_switch_parent(int on);
...
static int serial_imx_probe(struct platform_device *pdev)
{
	...
	//ttymxc0
	if(strcmp("2020000.serial", pdev->name) == 0) {
		g_pdev = pdev;
	}
...
extern void bt_uart_switch_parent(int on);
void uart_sleep_and_switch_parent(int k)
{
	pm_message_t m;
	serial_imx_suspend(g_pdev, m);
	g_BTstatus=k;
	msleep(100);
	bt_uart_switch_parent(k);
	msleep(100);
	serial_imx_resume(g_pdev);
}
EXPORT_SYMBOL(uart_sleep_and_switch_parent);
