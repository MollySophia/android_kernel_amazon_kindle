...
static void pxp_set_lut(struct pxps *pxp)
{
	...
	if (lut_op == PXP_LUT_NONE) {
		...
		for (pix_val = 0; pix_val < 256; pix_val += 4) {
			for (i = 0; i < 4; i++) {
				...
				if (entry_src <= 0x11) entry[i] = 0x00;
				else if (entry_src <= 0x88) entry[i] = 0x55;
				else if (entry_src < 0xEE) entry[i] = 0xAA;
				else entry[i] = 0xFF;
				if ((lut_op & PXP_LUT_REAGL) != 0)
					entry[i] &= 0xF0;
			}
	...
	}else if ((lut_op & PXP_LUT_REAGL) != 0) {
		/* Fill out LUT table with 8-bit monochromized values */
