...
static __init int bd7181x_probe(struct platform_device *pdev)
{
	...
	bd7181x_reg_write(pmic->mfd, BD7181X_REG_BUCK1_VOLT_L, BD7181X_BUCK1_09V_SUSPEND); // Set BUCK_1 suspend voltage to 0.9V
	bd7181x_reg_write(pmic->mfd, BD7181X_REG_CHG_SET1, BD7181X_WDT_MANUAL); // Set WDT_AUTO to 0 so we can have manual control 
	bd7181x_clear_bits(pmic->mfd, BD7181X_REG_GPO, READY_FORCE_LOW); // Turn OFF the green LED
