...
int dhd_tcpka_conn_enable(dhd_pub_t *dhd, int sess_id, int enable)
{
	...
	tcpka_t tcpka_conn;
	memset(&tcpka_conn, 0, sizeof(tcpka_conn));

	/* Store session id */
	tcpka_conn.sess_id = sess_id;

	/* Store Enable/Disable flag */
	tcpka_conn.flag = enable;

	iovar_len = bcm_mkiovar("tcpka_conn_enable", (char *)&tcpka_conn,
			sizeof(tcpka_t), iovbuf, sizeof(iovbuf));
	...
	retcode = dhd_wl_ioctl_cmd(dhd, WLC_GET_VAR, iovbuf, iovar_len,
							 FALSE, 0);
	retcode = retcode >= 0 ? 0 : retcode;
	if (retcode)
		DHD_TRACE(("%s: failed to set TCP KEEPALIVE to 0x%x, retcode = %d\n",
				__FUNCTION__, enable, retcode));
	else if (!tcpka_conn.flag) {
		mutex_lock(&dhd->tcpka_lock);
		memcpy(&dhd->tcpka[sess_id-1], iovbuf, sizeof(tcpka_t));
		mutex_unlock(&dhd->tcpka_lock);
		DHD_TRACE(("%s: successfully set TCPKEEPALIVE to 0x%x\n",
				__FUNCTION__, enable));
	}
	return retcode;
}
