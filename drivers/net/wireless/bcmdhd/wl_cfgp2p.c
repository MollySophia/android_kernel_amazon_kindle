            ...
            chanspec_t chspec)
{
	...
	CFGP2P_DBG(("---cfg p2p_ifadd "MACDBG_OBFUSCATE" %s %u\n",
		MAC2STRDBG_OBFUSCATE(ifreq.addr.octet),
...
wl_cfgp2p_ifdisable(struct bcm_cfg80211 *cfg, struct ether_addr *mac)
{
	...
	CFGP2P_INFO(("------primary idx %d : cfg p2p_ifdis "MACDBG_OBFUSCATE"\n",
		netdev->ifindex, MAC2STRDBG_OBFUSCATE(mac->octet)));
...
wl_cfgp2p_ifdel(struct bcm_cfg80211 *cfg, struct ether_addr *mac)
{
	...
	CFGP2P_INFO(("------primary idx %d : cfg p2p_ifdel "MACDBG_OBFUSCATE"\n",
	    netdev->ifindex, MAC2STRDBG_OBFUSCATE(mac->octet)));
            ...
            chanspec_t chspec)
{
	...
	CFGP2P_INFO(("---cfg p2p_ifchange "MACDBG_OBFUSCATE" %s %u"
		" chanspec 0x%04x\n", MAC2STRDBG_OBFUSCATE(ifreq.addr.octet),
		(if_type == WL_P2P_IF_GO) ? "go" : "client",
		(chspec & WL_CHANSPEC_CHAN_MASK) >> WL_CHANSPEC_CHAN_SHIFT,
		ifreq.chspec));
...
wl_cfgp2p_ifidx(struct bcm_cfg80211 *cfg, struct ether_addr *mac, s32 *index)
{
	...
	CFGP2P_INFO(("---cfg p2p_if "MACDBG_OBFUSCATE"\n", MAC2STRDBG_OBFUSCATE(mac->octet)));
