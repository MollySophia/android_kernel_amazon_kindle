 ...
 *
 * $Id: dhd_linux.c 567549 2015-06-29 23:34:48Z $
 */
...
#ifdef CONFIG_LAB126
#include <linux_osl.h>
uint8 *fw_image_unaligned = NULL, *fw_image = NULL;
int fw_image_size = 0;
 ...
 */
#ifdef CONFIG_LAB126
#define BRCM_4343W_FIRMWARE "/system/vendor/firmware/fw_bcmdhd.bin"
#define BRCM_4343W_NVRAM "/system/vendor/firmware/nvram.txt"
char firmware_path[MOD_PARAM_PATHLEN] = BRCM_4343W_FIRMWARE;
char nvram_path[MOD_PARAM_PATHLEN] = BRCM_4343W_NVRAM;
#else /* !CONFIG_LAB126 */
...
static int dhd_pm_callback(struct notifier_block *nfb, unsigned long action, void *ignored)
{
	int ret = NOTIFY_DONE;
	bool suspend = FALSE;
	dhd_info_t *dhdinfo = (dhd_info_t*)container_of(nfb, dhd_info_t, pm_notifier);
	...
	case PM_SUSPEND_PREPARE:
		...
		if (dhdinfo)
			dhd_suspend_resume_helper(dhdinfo, 1, 0);
		suspend = TRUE;
		break;
	...
	case PM_POST_SUSPEND:
		...
		if (dhdinfo)
			dhd_suspend_resume_helper(dhdinfo, 0, 0);
		break;
	}
...
char g_eapol_filter[25] = "106 0 0 12 0xFFFF 0x888E";
#endif /* LAB126_SUPPORT_CUSTOM_PKTFILTER */
...
static int dhd_set_suspend(int value, dhd_pub_t *dhd)
{
	...
	uint nd_ra_filter = 0;
	int ret = 0;
#ifdef LAB126_TCPKEEPALIVE_SUPPORT
	int i;
#endif
	...
	if (dhd->up) {
		if (value && dhd->in_suspend) {
...
#ifdef LAB126_TCPKEEPALIVE_SUPPORT
				for (i = 0; i < MAX_TCPKEEPALIVE_SESSIONS; i++)
					if (dhd->tcp_kepalive_set[i])
						/* Firmware session IDs are 1-4 */
						dhd_tcpka_conn_enable(dhd, i+1, 1);
#endif
				...
				for (i = 0; i < MAX_TCPKEEPALIVE_SESSIONS; i++)
					if (dhd->tcp_kepalive_set[i])
						dhd_tcpka_conn_enable(dhd, i+1, 0);
#endif
	...
	for (i = 0; pktbuf && i < numpkt; i++, pktbuf = pnext) {
		...
		if (protocol != ETHER_TYPE_BRCM) {
			if (dump_data[0] == 0xFF) {
				...
				DHD_ERROR(("%s: MULTICAST: " MACDBG_OBFUSCATE "\n",
					__FUNCTION__, MAC2STRDBG_OBFUSCATE(dump_data)));
			}
...
#endif /* ARP_OFFLOAD_SUPPORT */
#ifdef LAB126_TCPKEEPALIVE_SUPPORT
	memset(&dhd->pub.tcp_kepalive_set, 0, MAX_TCPKEEPALIVE_SESSIONS);
	mutex_init(&dhd->pub.tcpka_lock);
#endif
#ifdef CONFIG_IPV6
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	DHD_ERROR(("Firmware up: op_mode=0x%04x, MAC="MACDBG_OBFUSCATE"\n",
		dhd->op_mode, MAC2STRDBG_OBFUSCATE(dhd->mac.octet)));
	/* Set Country code  */
...
#ifdef LAB126_TCPKEEPALIVE_SUPPORT
	/* TCPKEEPALIVE feature */
	setbit(eventmask, WLC_E_WAKE_EVENT);
#endif /* LAB126_TCPKEEPALIVE_SUPPORT */
	...
	dhd->pktfilter_count = DHD_MAX_FILTER_SUPPORT;
#ifndef LAB126_SUPPORT_CUSTOM_PKTFILTER
	/* Setup filter to allow only unicast */
	dhd->pktfilter[DHD_UNICAST_FILTER_NUM] = "100 0 0 0 0x01 0x00";
#else
	/* Setup filter to allow only custom packet filter */
	for (p = 0; p < MAX_CUSTOM_FILTERS; p++)
		dhd->ipport_filterp[p] = NULL;

	dhd->pktfilter[DHD_UNICAST_FILTER_NUM] = NULL;
	dhd->pktfilter[DHD_EAPOL_FILTER_NUM] = g_eapol_filter;
#endif
...
dhd_register_if(dhd_pub_t *dhdp, int ifidx, bool need_rtnl_lock)
{
	...
	printf("Register interface [%s]  MAC: "MACDBG_OBFUSCATE"\n\n", net->name,
		MAC2STRDBG_OBFUSCATE(net->dev_addr));
...
#ifdef CONFIG_LAB126
			dhd_os_sdlock(dhdp);
			dhd_enable_oob_intr(dhd->pub.bus, FALSE);
#endif /* CONFIG_LAB126 */
...
#ifdef CONFIG_LAB126
				/*
				 * Some userspace process (healthd, for example)
				 * listens to NETLINK uevents. Unregister_netdev
				 * in suspend path triggers NETLINK dead lock
				 * because the process has been put in sleep at
				 * this moment.
				 * Ideally we should not suppress uevents in
				 * driver unload path. But differentiating them
				 * makes things complicated. Practically it
				 * doesn't seem to cauae any problem for upper
				 * layers when driver is unloaded.
				 */
				/* Do not send uevent on 'tx-0' */
				ifp->net->_tx->kobj.uevent_suppress = true;
				/* Do not send uevent on 'wlan0' */
				dev_set_uevent_suppress(&ifp->net->dev, true);
#endif /* CONFIG_LAB126 */
				unregister_netdev(ifp->net);
			}
			ifp->net = NULL;
...
extern void *brcm_wlan_mem_prealloc(int section, unsigned long size);
...
dhd_module_exit(void)
{
	...
	if (!fw_image)
		printk("%s: fw_image is NULL\n", __func__);
#ifndef CONFIG_DHD_USE_STATIC_BUF
	else
		MFREE(NULL, fw_image, FW_IMAGE_MAX + DHD_SDALIGN);
#endif
#endif /* CONFIG_LAB126 */
}
...
#ifdef CONFIG_LAB126
	fw_image_unaligned = fw_image =
#ifdef CONFIG_DHD_USE_STATIC_BUF
		brcm_wlan_mem_prealloc(DHD_PREALLOC_FW_BUF,
				       FW_IMAGE_MAX + DHD_SDALIGN);
#else
		MALLOC(NULL, FW_IMAGE_MAX + DHD_SDALIGN);
#endif
	if (!fw_image) {
		DHD_ERROR(("%s: Failed to allocate fw_image memory %d bytes\n",
			   __func__, FW_IMAGE_MAX + DHD_SDALIGN));
		return -ENOMEM;
	}
	if ((uint32)(uintptr)fw_image % DHD_SDALIGN)
		fw_image += (DHD_SDALIGN - ((uint32)(uintptr)fw_image % DHD_SDALIGN));
	printk("%s fw_image_unaligned=%p fw_image=%p\n", __func__, fw_image_unaligned, fw_image);
#endif /* CONFIG_LAB126 */
...
#ifdef CONFIG_LAB126
	if (wifi_card_enable() != 0) {
		printk(KERN_ERR "brcm 4343w driver could not enable wifi chip\n");
		return -ENODEV;
	}
#endif /* CONFIG_LAB126 */
...
#define FILTER_SRC_IPADDR               1
#define FILTER_DST_IPADDR               2
#define FILTER_SRC_PORT                 8
#define FILTER_DST_PORT                 16

#define FILTERID_LEN                    3
#define PATTERN_MASK_PORT_TYPE_LEN      16
#define PATTERN_PORT_TYPE_LEN           9
#define IPADDR_LEN                      16
#define PORT_LEN                        4

#define PATTERN_MASK_PROTTYPE_OFFSET    3
#define PATTERN_MASK_SRCIP_OFFSET       19
#define PATTERN_MASK_DSTIP_OFFSET       27
#define PATTERN_MASK_SRCPORT_OFFSET     35
#define PATTERN_MASK_DSTPORT_OFFSET     39

#define PATTERN_PROTTYPE_OFFSET         43
#define PATTERN_SRCIP_OFFSET            52
#define PATTERN_DSTIP_OFFSET            60
#define PATTERN_SRCPORT_OFFSET          68
#define PATTERN_DSTPORT_OFFSET          72


#define PATTERN_MASK_PROT_TYPE  " 0 0 23 0xFF0000"
#define PATTERN_MASK_IP         "FFFFFFFF"
#define PATTERN_MASK_NOIP       "00000000"
#define PATTERN_MASK_PORT       "FFFF"
#define PATTERN_MASK_NOPORT     "0000"

#define PATTERN_PROT_TYPE_UDP   " 0x110000"
#define PATTERN_PROT_TYPE_TCP   " 0x060000"

#define CUSTOM_FILTER_ID_BASE   107
#define PKTFILTER_ID_BASE       100

#define PROT_TYPE_UDP 17
#define PROT_TYPE_TCP 6

int net_os_rxfilter_add_custom_config(struct net_device *dev,
			char *extra, int len, int total_len, char *command)
{
	dhd_info_t *dhd = DHD_DEV_INFO(dev);
	int i, filter_sel = 0;
	uint32 src_port, dst_port;
	int  prot_type = -1, filter_id = -1;
	char ipaddr_str[IPV4_ADDR_STR_LEN];
	struct ipv4_addr src_ipaddr;
	struct ipv4_addr dst_ipaddr;
	char *ipport_filterp = NULL;

	if (!dhd || !extra || len == 0) {
		DHD_ERROR(("%s: Wrong custom filter config passed\n", __FUNCTION__));
		return -EINVAL;
	}

	for (i = 0; i < MAX_CUSTOM_FILTERS; i++) {
		if (dhd->pub.ipport_filterp[i] ==  NULL) {
			filter_id = CUSTOM_FILTER_ID_BASE + i;
			break;
		}
	}

	if (filter_id < 0) {
		DHD_ERROR(("%s: No free filters; All slots taken\n",
			   __func__));
		return -EINVAL;
	}

	if (sscanf(extra, "%d", &prot_type) != 1) {
		DHD_ERROR(("%s: sscanf error. check src ip addr\n",
				__func__));
		return -EINVAL;
	}

	if (prot_type != PROT_TYPE_UDP && prot_type != PROT_TYPE_TCP) {
		DHD_ERROR(("%s: Wrong Prot Type should be UDP(17) or TCP(6)",
			   " passed is %d\n", __func__, prot_type));
		return -EINVAL;
	}

	for (i = 0;  i < len && *(extra + i) != ' '; i++)
		;

	if (sscanf(extra + i + 1 , "%s", ipaddr_str) != 1) {
		DHD_ERROR(("%s: sscanf error. check src ip addr\n",
			   __func__));
		return -EINVAL;
	}

	if (strlen(ipaddr_str) < (IPV4_ADDR_STR_LEN / 2)) {
		DHD_INFO(("%s: src ip addr not configured\n", __func__));

	} else {
		if (!bcm_atoipv4(ipaddr_str, &src_ipaddr)) {
			DHD_ERROR(("%s:Invalid Source Ip address passed - %s\n",
				   __func__, ipaddr_str));
			return -EINVAL;
		}
		filter_sel |= FILTER_SRC_IPADDR;
	}

	for  (i = i+1;  i < len && *(extra + i) != ' '; i++)
		;

	if (sscanf(extra + i + 1 , "%s", ipaddr_str) != 1) {
		DHD_ERROR(("%s: sscanf error. check dst ip addr\n",
			   __func__));
		return -EINVAL;
	}

	if (strlen(ipaddr_str) < (IPV4_ADDR_STR_LEN / 2)) {
		DHD_INFO(("%s: Dst ip addr not configured\n", __func__));
	} else {
		if (!bcm_atoipv4(ipaddr_str, &dst_ipaddr)) {
			DHD_ERROR(("%s: Invalid Destination Ip address passed",
				   "- %s\n", __func__, ipaddr_str));
			return -EINVAL;
		}
		filter_sel |= FILTER_DST_IPADDR;
	}

	for (i = i + 1; i < len && *(extra + i) != ' '; i++);

	if (sscanf(extra + i + 1, "%d", &src_port) != 1) {
		DHD_ERROR(("%s: source port not read\n", __FUNCTION__));
		return -EINVAL;
	}

	for (i = i + 1;  i < len && *(extra + i) != ' '; i++);

	if (sscanf(extra + i + 1, "%d", &dst_port) != 1) {
		DHD_ERROR(("%s: Destination port not read\n", __FUNCTION__));
		return -EINVAL;
	}

	if (src_port)
		filter_sel |= FILTER_SRC_PORT;
	if (dst_port)
		filter_sel |= FILTER_DST_PORT;

	ipport_filterp = (char *)MALLOC(dhd->pub.osh, CUSTOM_FILTER_SIZE);

	memset(ipport_filterp , 0 , CUSTOM_FILTER_SIZE);

	snprintf(ipport_filterp, FILTERID_LEN + 1, "%03d",
		 filter_id);
	snprintf(ipport_filterp + PATTERN_MASK_PROTTYPE_OFFSET,
		 PATTERN_MASK_PORT_TYPE_LEN + 1, "%s", PATTERN_MASK_PROT_TYPE);
	snprintf(ipport_filterp + PATTERN_PROTTYPE_OFFSET,
		 PATTERN_PORT_TYPE_LEN + 1, "%s", (prot_type == PROT_TYPE_UDP) ?
		 PATTERN_PROT_TYPE_UDP : PATTERN_PROT_TYPE_TCP);

	if (filter_sel & FILTER_SRC_IPADDR) {
		snprintf(ipport_filterp + PATTERN_MASK_SRCIP_OFFSET,
			 IPADDR_LEN + 1, "%s", PATTERN_MASK_IP);
		snprintf(ipport_filterp + PATTERN_SRCIP_OFFSET,
			 IPADDR_LEN + 1, "%02X%02X%02X%02X",
			 src_ipaddr.addr[0], src_ipaddr.addr[1],
			 src_ipaddr.addr[2], src_ipaddr.addr[3]);
	} else {
		snprintf(ipport_filterp + PATTERN_MASK_SRCIP_OFFSET,
			 IPADDR_LEN + 1, "%s", PATTERN_MASK_NOIP);
		snprintf(ipport_filterp + PATTERN_SRCIP_OFFSET,
			 IPADDR_LEN + 1, "%s", PATTERN_MASK_NOIP);
	}

	if (filter_sel & FILTER_DST_IPADDR) {
		snprintf(ipport_filterp + PATTERN_MASK_DSTIP_OFFSET,
			 IPADDR_LEN + 1, "%s", PATTERN_MASK_IP);
		snprintf(ipport_filterp + PATTERN_DSTIP_OFFSET,
			 IPADDR_LEN + 1, "%02X%02X%02X%02X",
			 dst_ipaddr.addr[0], dst_ipaddr.addr[1],
			 dst_ipaddr.addr[2], dst_ipaddr.addr[3]);
	} else {
		snprintf(ipport_filterp + PATTERN_MASK_DSTIP_OFFSET,
			 IPADDR_LEN + 1, "%s", PATTERN_MASK_NOIP);
		snprintf(ipport_filterp + PATTERN_DSTIP_OFFSET,
	...
	if (filter_sel & FILTER_SRC_PORT) {
		snprintf(ipport_filterp + PATTERN_MASK_SRCPORT_OFFSET,
			 PORT_LEN + 1, "%s", PATTERN_MASK_PORT);
		snprintf(ipport_filterp + PATTERN_SRCPORT_OFFSET,
			 PORT_LEN + 1, "%04X", src_port);
	} else {
		snprintf(ipport_filterp + PATTERN_MASK_SRCPORT_OFFSET,
			 PORT_LEN + 1, "%s", PATTERN_MASK_NOPORT);
		snprintf(ipport_filterp + PATTERN_SRCPORT_OFFSET,
			 PORT_LEN + 1, "%s", PATTERN_MASK_NOPORT);
	}

	if (filter_sel & FILTER_DST_PORT) {
		snprintf(ipport_filterp + PATTERN_MASK_DSTPORT_OFFSET,
			 PORT_LEN + 1, "%s", PATTERN_MASK_PORT);
		snprintf(ipport_filterp + PATTERN_DSTPORT_OFFSET,
			 PORT_LEN + 1, "%04X", dst_port);
	} else {
		snprintf(ipport_filterp + PATTERN_MASK_DSTPORT_OFFSET,
			 PORT_LEN + 1, "%s", PATTERN_MASK_NOPORT);
		snprintf(ipport_filterp + PATTERN_DSTPORT_OFFSET,
	...
	*(ipport_filterp + PATTERN_PROTTYPE_OFFSET) = ' ';

	for (i = 0; i < MAX_CUSTOM_FILTERS; i++) {
		if (dhd->pub.ipport_filterp[i] != NULL) {
			if (!strcmp(dhd->pub.ipport_filterp[i]
				+ PATTERN_PROTTYPE_OFFSET,
				ipport_filterp + PATTERN_PROTTYPE_OFFSET)) {
				DHD_ERROR(("%s:Duplicate custom filter",
					   " config\n", __func__));
				MFREE(dhd->pub.osh, ipport_filterp,
				      CUSTOM_FILTER_SIZE);
				return i + CUSTOM_FILTER_ID_BASE;
			}
		}
	}
	dhd->pub.pktfilter[filter_id - PKTFILTER_ID_BASE] = ipport_filterp;
	dhd->pub.ipport_filterp[filter_id - CUSTOM_FILTER_ID_BASE] =
		ipport_filterp;
	dhd_pktfilter_offload_set(&dhd->pub,
				  dhd->pub.pktfilter[filter_id -
				  PKTFILTER_ID_BASE]);

	return snprintf(command, total_len, "filter_id = %d", filter_id);
}

int net_os_rxfilter_del_custom_config(struct net_device *dev,
			char *extra, int len)
{
	dhd_info_t *dhd = DHD_DEV_INFO(dev);
	int filter_id = -1, filter_num = 0;

	if (!dhd || !extra || len == 0) {
		DHD_ERROR(("%s: Wrong custom filter config",
			   " passed\n", __func__));
		return -EINVAL;
	}

	if (sscanf(extra, "%d", &filter_id) != 1) {
		DHD_ERROR(("%s: sscanf error. check filter_id\n",
			   __func__));
		return -EINVAL;
	}

	if ((filter_id < CUSTOM_FILTER_ID_BASE) ||
	    (filter_id >= (MAX_CUSTOM_FILTERS + CUSTOM_FILTER_ID_BASE))) {
		DHD_ERROR(("%s: Invalid  filter_id\n", __func__));
		return -EINVAL;
	}

	filter_num = filter_id - PKTFILTER_ID_BASE;
	if (dhd->pub.pktfilter[filter_num] != NULL) {
		dhd_pktfilter_offload_delete(&dhd->pub, filter_id);
		MFREE(dhd->pub.osh, dhd->pub.ipport_filterp[filter_id -
			CUSTOM_FILTER_ID_BASE], CUSTOM_FILTER_SIZE);
		dhd->pub.ipport_filterp[filter_id - CUSTOM_FILTER_ID_BASE]
			= NULL;
		dhd->pub.pktfilter[filter_num] = NULL;
	}
	return 0;
}
#endif
...
int net_os_rxfilter_add_remove(struct net_device *dev, int add_remove, int num)
{
...
#ifdef LAB126_SUPPORT_CUSTOM_PKTFILTER
	if (!dhd || (num == DHD_MDNS_FILTER_NUM))
		return ret;
#else
	...
	switch (num) {
#ifdef LAB126_SUPPORT_CUSTOM_PKTFILTER
		case DHD_UNICAST_FILTER_NUM:
			filterp = "100 0 0 0 0x01 0x00";
			filter_id = 100;
			break;
#endif
...
#ifdef LAB126_TCPKEEPALIVE_SUPPORT
void net_os_update_dhd_tcpkeepalive(struct net_device *dev, int val,
	int enable)
{
	dhd_info_t *dhd = DHD_DEV_INFO(dev);
	dhd_pub_t *pub = &dhd->pub;
	pub->tcp_kepalive_set[val] = enable;
	return;
}

int net_os_set_get_tcpka_info(struct net_device *dev, tcpka_t *tcpka_res,
	int sess_id, int set)
{
	dhd_info_t *dhd = DHD_DEV_INFO(dev);
	dhd_pub_t *pub = &dhd->pub;

	if (!pub->tcp_kepalive_set[sess_id])
		return -EINVAL;

	mutex_lock(&pub->tcpka_lock);
	if (set)
		memcpy(&dhd->pub.tcpka[sess_id], tcpka_res, sizeof(tcpka_t));
	else
		memcpy(tcpka_res, &dhd->pub.tcpka[sess_id], sizeof(tcpka_t));

	mutex_unlock(&pub->tcpka_lock);
	return 0;
}
