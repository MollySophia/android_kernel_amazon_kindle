...
#ifdef CONFIG_LAB126
static ssize_t udc_connected_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct usb_udc		*udc = container_of(dev, struct usb_udc, dev);
	struct usb_gadget	*gadget = udc->gadget;
...
static DEVICE_ATTR(connected, S_IRUGO, udc_connected_show, NULL);
int usb_udc_connected()
{
	if(!my_udc_gadget)
	{
