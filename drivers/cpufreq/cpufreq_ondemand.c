...
#define DEF_OVERRIDE_DURATION_MS    (600)
...
enum {
	OVERRIDE_NONE = 0, 	/* No override active */
	OVERRIDE_GRACE,		/* Override in the grace period */
	OVERRIDE_FULL,		/* Override is in full effect */
};

static void update_override_timer(struct od_dbs_tuners *tuners);
#endif
		...
		unsigned int is_override)
{
	...
	struct od_cpu_dbs_info_s *this_dbs_info = &per_cpu(od_cpu_dbs_info, cpu);
	...
	if (is_override > 0)
		is_override = OVERRIDE_FULL;

	mutex_lock(&dbs_data->mutex);

	od_tuners->override = is_override;
	if (is_override)
	{
		/* Reset the override timer */ 
		od_tuners->override_timeout = jiffies + msecs_to_jiffies(od_tuners->override_delay);
		od_tuners->up_threshold = min(od_tuners->override_up_threshold, (unsigned int)MICRO_FREQUENCY_UP_THRESHOLD);
...
static void od_dbs_timer(struct work_struct *work)
{
	...
	if (od_tuners->override == OVERRIDE_FULL)
	{
		struct cpufreq_policy *policy = dbs_info->cdbs.cur_policy;
		dbs_freq_increase(policy, policy->max);
		schedule_delayed_work_on(cpu, &dbs_info->cdbs.work, usecs_to_jiffies(od_tuners->sampling_rate));
...
#ifdef CONFIG_CPU_FREQ_OVERRIDE_LAB126
static ssize_t store_override(struct dbs_data *dbs_data,
		const char *buf, size_t count)
{
	struct od_dbs_tuners *od_tuners = dbs_data->tuners;
	unsigned int input;
...
static ssize_t store_override_delay(struct dbs_data *dbs_data,
		const char *buf, size_t count)
{
	struct od_dbs_tuners *od_tuners = dbs_data->tuners;
	unsigned int input;
...
static ssize_t store_override_grace_delay(struct dbs_data *dbs_data,
		const char *buf, size_t count)
{
	struct od_dbs_tuners *od_tuners = dbs_data->tuners;
	unsigned int input;
...
static ssize_t store_override_up_threshold(struct dbs_data *dbs_data,
		const char *buf, size_t count)
{
	struct od_dbs_tuners *od_tuners = dbs_data->tuners;
	unsigned int input;
...
static void update_override_timer(struct od_dbs_tuners *tuners)
{
	if (tuners->override == OVERRIDE_FULL)
	{
		/* Override has expired. Switch to grace period */
		if (jiffies > tuners->override_timeout)
		{
			tuners->override = OVERRIDE_GRACE;
			tuners->override_timeout = jiffies + msecs_to_jiffies(tuners->override_grace_delay);
		}
	}
	else if (tuners->override == OVERRIDE_GRACE)
	{
