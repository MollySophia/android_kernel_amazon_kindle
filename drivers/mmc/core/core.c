...
#ifdef CONFIG_LAB126
	if (host->card !=NULL) {
		if (host->card->type == MMC_TYPE_SDIO) {
			/* For SDIO card, Set signal voltage to 1.8V */
			__mmc_set_signal_voltage(host, MMC_SIGNAL_VOLTAGE_180);
		}
	} else
#endif	
	/* For other cards, Set signal voltage to 3.3V */
					    ...
					    unsigned int arg)
{
	...
	if (!qty)
		return 0;
#ifdef CONFIG_LAB126
	/*for some slow eMMC, qty could be 1 here, need to convert 1 to sectors instead of just return 1*/
	if (qty !=1 && !mmc_card_sd(card)) qty--;
...
void mmc_rescan(struct work_struct *work)
{
...
#else
	/* only change for mmc1 */
	if((host->index == 0) && (host->caps & MMC_CAP_NONREMOVABLE) &&
		host->rescan_entered) {
		return;
	}
#endif
	 ...
	 */
#ifdef CONFIG_LAB126
	/* trigger card detect for mmc1 if function pointer is registered */
	if (host->bus_ops && host->bus_ops->detect && !host->bus_dead
		&& (!(host->caps & MMC_CAP_NONREMOVABLE) || (host->index == 1))) {
