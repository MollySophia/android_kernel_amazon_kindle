...
struct sdhci_host *mmc1_host = NULL;
EXPORT_SYMBOL(mmc1_host);
			 ...
			 struct esdhc_platform_data *boarddata)
{
	...
	/* used in wifi_card_disable & wifi_card_enable */
	if(host != NULL && host->mmc && host->mmc->index == 1)
		mmc1_host = host;
	...
	if (of_find_property(np, "fsl,sig-voltage-switch-disable", NULL))
		boarddata->no_sig_voltage_switch = true;
	else
		boarddata->no_sig_voltage_switch = false;
#endif
	return 0;
}
#else
...
static int sdhci_esdhc_imx_probe(struct platform_device *pdev)
{
	...
	if (boarddata->no_sig_voltage_switch == true)
...
#ifdef CONFIG_LAB126
	/* Lab126: disable HS200 by default */
	host->quirks2 &= ~SDHCI_QUIRK2_ALLOW_HS200;
#endif
