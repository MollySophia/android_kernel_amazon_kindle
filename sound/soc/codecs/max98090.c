...
static const struct snd_soc_dapm_route max98090_dapm_routes[] = {
	...
	{"MICBIAS", NULL, "SHDN"},
	{"HP Left Out", NULL, "SHDN"},
	{"HP Right Out", NULL, "SHDN"},
	{"SPK Left Out", NULL, "SHDN"},
	{"SPK Right Out", NULL, "SHDN"},
	{"RCV Left Out", NULL, "SHDN"},
	{"RCV Right Out", NULL, "SHDN"},
...
static int max98090_add_widgets_0(struct snd_soc_codec *codec)
{
...
static int max98090_add_widgets_1(struct snd_soc_codec *codec)
{
	struct snd_soc_dapm_context *dapm = &codec->dapm;
...
static void max98090_configure_bclk(struct snd_soc_codec *codec)
{
	...
	pr_info("%s:MAX98090 max98090_configure_bclk=%d, lrclk=%d\n",
		__func__, max98090->bclk, max98090->lrclk);
...
static int max98090_find_closest_pclk(int pclk)
{
	...
	return ARRAY_SIZE(dmic_table)-1;
}
		...
		int target_dmic_clk, int pclk, int fs)
{
	...
	dmic_comp = dmic_table[pclk_index].comp_values[micclk_index][i];

	dmic_freq_actual = pclk / dmic_divisors[micclk_index];
	if (dmic_freq_actual < 3500000)
		dmic_freq = 0;
	else if (dmic_freq_actual < 4500000)
		dmic_freq = 1;
	else
		dmic_freq = 2;
				   ...
				   struct snd_soc_dai *dai)
{
	...
	pr_info("%s:MAX98090 bclk=%d, lrclk=%d\n",
		__func__, max98090->bclk, max98090->lrclk);
	...
	case SNDRV_PCM_FORMAT_S24_LE:
		snd_soc_update_bits(codec, M98090_REG_INTERFACE_FORMAT,
			M98090_WS_MASK, MS98090_WS_24BITS);
		break;
	default:
		return -EINVAL;
	}
...
int max98090_read_jack(struct snd_soc_codec *codec)
{
	int ret = -1;

	dev_info(codec->dev, "max98090_read_jack\n");

	if (codec) {
		ret = snd_soc_read(codec, M98090_REG_JACK_STATUS);
		if (ret < 0)
			dev_err(codec->dev, "Failed to read jack: %d\n",
				ret);
		else
		ret &= (M98090_LSNS_MASK|M98090_JKSNS_MASK);
	}

	return ret;
}
EXPORT_SYMBOL_GPL(max98090_read_jack);
...
static int max98090_probe(struct snd_soc_codec *codec)
{
	...
	if ((ret >= M98090_REVA) && (ret <= M98090_REVA + 0x0f)) {
		...
		dev_info(codec->dev, "MAX98090 ID = 0x%02X\n", ret);
	} else if ((ret >= M98091_REVA) && (ret <= M98091_REVA + 0x0f)) {
		max98090->devtype = MAX98091;
		dev_info(codec->dev, "MAX98091 ID = 0x%02X\n", ret);
	} else {
		max98090->devtype = MAX98090;
		dev_err(codec->dev, "Unrecognized ID 0x%02X\n", ret);
	}
	...
	if (max98090->irq) {
		/* TODO: Enable interrupts once correct irq is configured in DTS
	 ...
	 * An old interrupt occurring prior to installing the ISR
	...
	/* To avoid a stack problem in 3.8.5 add widgets in two calls */
	/* First add 98090 widgets */
	max98090_add_widgets_0(codec);
	/* If it's a 98091 then add 98091 widgets */
	if (max98090->devtype == MAX98091)
		max98090_add_widgets_1(codec);
